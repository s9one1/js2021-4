...
## [06월 1일]
>### JQuery 사용 준비
- 공식 웹 사이트의 주소는 http://hquery.com/이다.
- JQuery 다운로드 페이지에서 jQuery 파일을 다운로드하거나 cdn경로를 구할 수 있다.
>### jQuery 객체
- $함수를 활용한다.
- $ 함수의 매개 변수에는 문서 객체, CSS 형식, HTML 형식의 문자열을 삽입한다.
    - $(<매개 변수>).메소드(<매개 변수>,<매개 변수>)
>### 문서 객체 선택
- parent(), find() 메소드를 사용해 부모나 후손을 선택할 수 있다.
>### 문서 객체 개별 조작
- $()함수를 사용하면 여러 개의 문서 객체를 선택할 수 있다.
- 문서 객체가 몇 개 선택되었는지 확인할 때는 length 속성을 사용한다.
- 배열처럼 대괄호를 사용해서 요소 하나를 꺼낼 수 도 있고, get() 메소드를 사용햇 꺼낼 수도 있다.
>### 문서 객체 조작
- jQuery를 사용하면 문서 객체를 굉장히 쉽게 조작할 수 있다.
>#### 문자 조작
-문서 객체의 문자를 조작할 때 text(), html()메소드를 사용한다.
>#### 스타일 조작
- 문서 객체의 스타일을 조작할 때는 css() 메소드를 사용하낟.
- get 형태와 set 형태를 나누어 사용하낟.
- get 형태는 선택자로 선택된 문서 객체 중 첫 번째 문서 객체의 스타일을 가져온다.
>### 문서 객체 생성 
- 문서 객체를 생성할 때는 $() 함수의 매개 변수에 HTML 형식의 문자열을 입력한다.
>### 이벤트
- jQuery가 기본적으로 제공하는 이벤트 관련 메소드는 on(), off()이다.
- 이벤트를 연결하는 방법에는 직접연결과 간접연결 방법이 있다.
>#### 이벤트 직접 연결
- 특정 태그에 이벤트를 연결하고, 특정 태그를 눌렀을 때 이벤트가 발생하게 되는것을 의미한다.
- 직접 연결할 때는 on() 메소드를 이용한다.
- 콜백 함수 내부에서 this 키워드는 이벤트를 발생시키는 객체이다.
>#### 이벤트 간접 연결
- 부모에게 이벤트를 위임햇 부모가 이벤트를 처리하게 하는것이다.
>#### 이벤트 제거
